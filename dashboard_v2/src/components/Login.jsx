...
export default function Login({onLogin}){
  ...
  return <div className="glass-card login-panel"><div className="login-brand"><EpicVMMark size={42}/><div><strong>{MANAGER_NAME.split(' ')[0]}</strong><span>{MANAGER_NAME.split(' ').slice(1).join(' ')}</span></div></div><div className="login-heading"><h2>Welcome back</h2><p>{BRAND_TAGLINE}</p></div><form onSubmit={submit}>
