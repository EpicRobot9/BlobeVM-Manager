...
INSTALL_ROOT="${EPICVM_ROOT:-${BLOBEVM_ROOT:-/opt/blobe-vm}}"
REPO_DIR="${INSTALL_ROOT}/repo"
INSTALLER_REL="server/install.sh"
LOGFILE="${EPICVM_BOOTSTRAP_LOG:-${BLOBEVM_BOOTSTRAP_LOG:-/var/log/blobe-vm-bootstrap.log}}"
ASSUME_YES=0
SKIP_PULL=0
DRY_RUN=0
INSTALLER_ARGS=()
...
# EPICVM_* is the public interface; legacy BLOBEVM_* names remain downstream-facing.
export BLOBEVM_ASSUME_DEFAULTS="${EPICVM_ASSUME_DEFAULTS:-${BLOBEVM_ASSUME_DEFAULTS:-1}}"
export BLOBEVM_ENABLE_DASHBOARD="${EPICVM_ENABLE_DASHBOARD:-${BLOBEVM_ENABLE_DASHBOARD:-1}}"
export BLOBEVM_AUTO_CREATE_VM="${EPICVM_AUTO_CREATE_VM:-${BLOBEVM_AUTO_CREATE_VM:-1}}"
export BLOBEVM_INITIAL_VM_NAME="${EPICVM_INITIAL_VM_NAME:-${BLOBEVM_INITIAL_VM_NAME:-testvm}}"
# Default to direct mode unless explicitly overridden; avoids 80/443 conflicts on random VPSes.
export BLOBEVM_NO_TRAEFIK="${EPICVM_NO_TRAEFIK:-${BLOBEVM_NO_TRAEFIK:-1}}"
export BLOBEVM_FORCE_REBUILD="${EPICVM_FORCE_REBUILD:-${BLOBEVM_FORCE_REBUILD:-0}}"
